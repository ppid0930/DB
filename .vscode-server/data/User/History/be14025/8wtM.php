...
// 게시글 정보 가져오기
$sql_posts = "SELECT WRITE_ID AS write_id, CONTENT_NAME AS title, TO_CHAR(CREATED_AT, 'YYYY-MM-DD') AS created_at
              FROM WRITE
              WHERE USER_ID = :user_id 
...
if (!oci_execute($stmt_posts)) {
    die("게시글 정보를 가져올 수 없습니다: " . oci_error($stmt_posts)['message']);
}

$posts = [];
while ($row = oci_fetch_assoc($stmt_posts)) {
    $posts[] = $row; // 모든 데이터를 배열에 추가
}
                    ...
                    <tr>
                        ...
                        <td>
                            <form action="delete_post.php" method="post" style="margin:0;">
                                <input type="hidden" name="post_id" value="<?php echo htmlspecialchars($post['write_id']); ?>">
                                <input type="hidden" name="user_id" value="<?php echo htmlspecialchars($user_id); ?>">
