...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // SQL 쿼리 작성: 댓글 추가
    $sql = "INSERT INTO comments (post_id, write_id, comment_content, comment_date) VALUES (:post_id, :write_id, :comment_content, SYSDATE)";
    ...
    // 실행
    if (oci_execute($stid)) {
        echo "<script>alert('댓글이 성공적으로 추가되었습니다.'); window.location.href='board_view.php?post_id=2';</script>";
    } else {
        $e = oci_error($stid);
        echo "<script>alert('댓글 추가에 실패했습니다.'); history.back();</script>";
    }
...
<head>
    ...
    <title>댓글 추가 - 홍보게시판</title>
</head>
<body>
    <h2>댓글 작성</h2>
    <form action="promotion_add_comment.php" method="post">
