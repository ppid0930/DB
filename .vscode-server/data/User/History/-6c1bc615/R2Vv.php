...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        // 시퀀스를 사용하여 새 COMMENT_ID 값 생성
        $seq_sql = "SELECT COMMENT_T_SEQ.NEXTVAL AS NEW_ID FROM DUAL";
        $seq_stmt = oci_parse($conn, $seq_sql);

        if (!oci_execute($seq_stmt)) {
            $e = oci_error($seq_stmt);
            throw new Exception("시퀀스 NEXTVAL 조회 실패: " . $e['message']);
        }

        $seq_row = oci_fetch_assoc($seq_stmt);
        if (!$seq_row || !isset($seq_row['NEW_ID'])) {
            throw new Exception("COMMENT_ID 생성 실패: 시퀀스에서 값을 가져오지 못했습니다.");
        }

        $comment_id = $seq_row['NEW_ID']; // 새로 생성된 COMMENT_ID
        ...
        // 리소스 정리
        oci_free_statement($seq_stmt ?? null);
