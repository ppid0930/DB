...
// 게시글 가져오기 (번호는 오래된 순으로 부여, 최신순으로 출력)
$write_sql = "
    SELECT TEMP.NUM AS NUMBER,
           TEMP.WRITE_ID, 
           TEMP.CONTENT_NAME, 
           TEMP.USER_ID, 
           TO_CHAR(TEMP.CREATED_AT, 'YYYY-MM-DD HH24:MI:SS') AS CREATED_AT
    FROM (
        SELECT ROW_NUMBER() OVER (ORDER BY CREATED_AT ASC) AS NUM, -- 번호는 오래된 순으로 부여
               WRITE_ID, CONTENT_NAME, USER_ID, CREATED_AT
        FROM WRITE
        WHERE POST_ID = :post_id
    ) TEMP
    ORDER BY TEMP.CREATED_AT DESC -- 최신순으로 출력
            ...
            <tbody>
                ...
                if (empty($posts)) {
                    ...
                    foreach ($posts as $post) {
                        echo "<tr>
                            <td>" . htmlspecialchars($post['NUMBER']) . "</td>
