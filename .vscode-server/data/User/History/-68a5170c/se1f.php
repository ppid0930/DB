...
// 현재 POST_ID에서 가장 큰 WRITE_ID 구하기
$get_max_id_sql = "SELECT NVL(MAX(TO_NUMBER(WRITE_ID)), 0) + 1 AS NEXT_ID 
                   FROM WRITE 
                   WHERE POST_ID = :post_id";
$get_max_id_stmt = oci_parse($conn, $get_max_id_sql);
oci_bind_by_name($get_max_id_stmt, ":post_id", $post_id);
oci_execute($get_max_id_stmt);
$row = oci_fetch_assoc($get_max_id_stmt);
$next_write_id = str_pad($row['NEXT_ID'], 3, '0', STR_PAD_LEFT); // 3자리 문자열로 포맷팅
oci_free_statement($get_max_id_stmt);
...
if (oci_execute($stmt)) {
    ...
    echo "<script>
        alert('게시글이 작성되었습니다.');
        window.location.href = 'promotion_board.php?post_id=$post_id';
    </script>";
    exit;
} else {
