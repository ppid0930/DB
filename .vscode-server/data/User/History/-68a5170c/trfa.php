...
// 게시글 가져오기 (번호를 오래된 게시물부터 부여하되 최신 게시물이 위로 정렬되도록)
$write_sql = "
    SELECT NUM, WRITE_ID, CONTENT_NAME, USER_ID, CREATED_AT
    FROM (
        SELECT ROW_NUMBER() OVER (ORDER BY CREATED_AT ASC) AS NUM,
    ...
    ORDER BY NUM DESC -- 최신 게시물이 위로 오도록 정렬
";
...
oci_bind_by_name($write_stmt, ":end_row", $start_row + $limit);
