...
$write_sql = "
    ...
    FROM (
        SELECT ROW_NUMBER() OVER (ORDER BY CREATED_AT ASC) AS NUM, -- 오래된 순으로 번호 부여
...
$write_sql .= "
    ...
    ORDER BY CREATED_AT DESC -- 최신순으로 화면에 표시
";
...
<body>
    ...
    <div class="search-container">
        <form action="promotion_board.php" method="get">
            ...
            <tbody>
                ...
                if (empty($posts)) {
                    ...
                    foreach ($posts as $post) {
                        echo "<tr>
                            <td>{$post['NUM']}</td>
                            <td><a href='promotion_write.php?write_id={$post['WRITE_ID']}'>" . htmlspecialchars($post['CONTENT_NAME']) . "</a></td>
