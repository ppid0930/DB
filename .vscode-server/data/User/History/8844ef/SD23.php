...
if ($comment_id <= 0 || empty($new_content)) {
    echo json_encode(['success' => false, 'error' => '유효하지 않은 요청입니다.']);
    exit;
}
...
if (!$row || $row['USER_ID'] !== $_SESSION['user_id']) {
    echo json_encode(['success' => false, 'error' => '본인이 작성한 댓글만 수정할 수 있습니다.']);
    exit;
}
...
if (oci_execute($update_stmt)) {
    ...
    echo json_encode(['success' => false, 'error' => htmlspecialchars($error['message'])]);
}
