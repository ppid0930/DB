...
// WRITE_ID 생성 (시퀀스 활용)
$get_write_id_sql = "SELECT NVL(MAX(TO_NUMBER(WRITE_ID)), 0) + 1 AS WRITE_ID FROM WRITE";
$write_id_stmt = oci_parse($conn, $get_write_id_sql);
oci_execute($write_id_stmt);
$row = oci_fetch_assoc($write_id_stmt);
$write_id = str_pad($row['WRITE_ID'], 3, '0', STR_PAD_LEFT); // 3자리 문자열로 포맷팅
oci_free_statement($write_id_stmt);
...
if (oci_execute($stmt)) {
    ...
    oci_free_statement($stmt);
    oci_close($conn);
    die("게시글 작성 실패: " . htmlspecialchars($error['message']));
}
?>
