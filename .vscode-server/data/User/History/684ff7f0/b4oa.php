...
// 게시글 정보 가져오기
$post_sql = "SELECT CONTENT_NAME, CONTENT, USER_ID, TO_CHAR(CREATED_AT, 'YYYY-MM-DD HH24:MI:SS') AS CREATED_AT,POST_NAME 
...
// 게시판 이름 저장
$board_name = $post['POST_NAME']; // 게시판 이름 (예: "자유게시판" 또는 "홍보게시판")
...
<head>
    ...
    <style>
        ...
        .post-content {
            ...
            word-break: break-word; /* 긴 텍스트가 박스를 벗어나지 않도록 처리 */
        }
        .post-image {
            max-width: 100%;
            margin-top: 15px;
        ...
        .comment .delete-btn {
            ...
            cursor: pointer;
            margin-right: 15px; /* 댓글 내용과 버튼 간격 추가 */
        }
...
<body>
    ...
    <div class="container">
        <!-- 게시판 이름 표시 -->
        <div class="board-name" style="text-align: center; margin-bottom: 20px; font-size: 18px; font-weight: bold; color: #555;">
            ...
            <?php while ($row = oci_fetch_assoc($comments_stmt)) { ?>
                <div class="comment">
                    ...
                    <?php if ($row['USER_ID'] === $_SESSION['user_id']) { ?>
                        <a href="delete_comment.php?comment_id=<?php echo $row['COMMENT_ID']; ?>&write_id=<?php echo $write_id; ?>" class="delete-btn">삭제</a>
                    <?php } ?>
