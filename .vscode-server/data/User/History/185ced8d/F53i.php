...
// 이전 URL 가져오기 (Referer 사용 또는 hidden 필드 값)
$redirect_url = isset($_POST['redirect_url']) ? $_POST['redirect_url'] : (isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : 'mainpage.php');
...
if (oci_execute($insert_stmt)) {
    ...
    error_log("DB Error: " . print_r($error, true)); // 에러를 PHP 로그에 출력
    echo "<script>
        alert('오류 발생: 데이터 입력에 실패했습니다. 관리자에게 문의하세요.');
        history.back();
    </script>";
}
